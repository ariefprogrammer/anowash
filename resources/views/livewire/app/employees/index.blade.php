<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold">Karyawan</h1>
        <button wire:click="openCreateModal" class="bg-teal-600 text-white px-4 py-2 rounded hover:bg-teal-700">
            + Tambah Karyawan
        </button>
    </div>

    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-left">
                <tr>
                    <th class="px-4 py-3">Nama</th>
                    <th class="px-4 py-3">Outlet</th>
                    <th class="px-4 py-3">Tipe Gaji</th>
                    <th class="px-4 py-3">Status</th>
                    <th class="px-4 py-3">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y">
                @forelse($employees as $employee)
                    <tr>
                        <td class="px-4 py-3 font-medium">{{ $employee->name }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $employee->outlet->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-600">
                            {{ $employee->payroll_type === 'flat' ? 'Gaji Tetap' : 'Komisi' }}
                        </td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 rounded text-xs {{ $employee->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                {{ $employee->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 space-x-2">
                            <button wire:click="openEditModal({{ $employee->id }})" class="text-teal-600 hover:underline">Edit</button>
                            <button
                                wire:click="delete({{ $employee->id }})"
                                wire:confirm="Yakin ingin menghapus karyawan ini?"
                                class="text-red-600 hover:underline"
                            >
                                Hapus
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">Belum ada karyawan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    @if($showModal)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" wire:click.self="$set('showModal', false)">
            <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
                <h2 class="text-lg font-semibold mb-4">{{ $editingId ? 'Edit Karyawan' : 'Tambah Karyawan' }}</h2>

                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium mb-1">Nama</label>
                        <input type="text" wire:model="name" class="w-full border rounded px-3 py-2">
                        @error('name') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Outlet</label>
                        <select wire:model="outlet_id" class="w-full border rounded px-3 py-2">
                            <option value="">-- Pilih Outlet --</option>
                            @foreach($outlets as $outlet)
                                <option value="{{ $outlet->id }}">{{ $outlet->name }}</option>
                            @endforeach
                        </select>
                        @error('outlet_id') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">No. Telepon</label>
                        <input type="text" wire:model="phone" class="w-full border rounded px-3 py-2">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Alamat</label>
                        <textarea wire:model="address" rows="2" class="w-full border rounded px-3 py-2"></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Tanggal Bergabung</label>
                        <input type="date" wire:model="join_date" class="w-full border rounded px-3 py-2">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Tipe Gaji</label>
                        <select wire:model.live="payroll_type" class="w-full border rounded px-3 py-2">
                            <option value="commission">Komisi</option>
                            <option value="flat">Gaji Tetap</option>
                        </select>
                    </div>

                    @if($payroll_type === 'flat')
                        <div>
                            <label class="block text-sm font-medium mb-1">Nominal Gaji Tetap (Rp)</label>
                            <input type="number" step="0.01" wire:model="flat_salary" class="w-full border rounded px-3 py-2">
                            @error('flat_salary') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                        </div>
                    @endif

                    @if($allowanceTypes->count() > 0)
                        <div>
                            <label class="block text-sm font-medium mb-1">Tunjangan</label>
                            <div class="space-y-1 border rounded p-2 max-h-32 overflow-y-auto">
                                @foreach($allowanceTypes as $type)
                                    <label class="flex items-center gap-2 text-sm">
                                        <input type="checkbox" wire:model="selectedAllowanceTypes" value="{{ $type->id }}">
                                        {{ $type->name }} (Rp{{ number_format($type->amount, 0, ',', '.') }})
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    @endif

                    <label class="flex items-center gap-2 text-sm">
                        <input type="checkbox" wire:model="is_active">
                        Aktif
                    </label>

                    <div class="flex justify-end gap-2 pt-2">
                        <button type="button" wire:click="$set('showModal', false)" class="px-4 py-2 rounded border">Batal</button>
                        <button type="submit" class="px-4 py-2 rounded bg-teal-600 text-white hover:bg-teal-700">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    @endif
</div>